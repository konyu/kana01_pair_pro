...
class Kana01
  ...
  def test
	...
	def has_kigo(arr, kigo)
		arr.each do |a|
			return true if a == kigo
		end
		false
	end
	...
	def keisan(arr, kigo)
		...
		kekka = nil
		arr_num = nil
		arr.each_with_index do |nakami, i|
			if nakami == kigo
				arr_num = i
				kekka = eval("#{arr[i-1]} #{kigo} #{arr[i+1]}")
			end
		end
		# 配列の　計算結果があるから　3つへって、一つになるようにする
		unless arr_num.nil?
			arr.slice!(arr_num-1, 3)
			arr.insert(arr_num-1, kekka)
		end

		# 再帰処理
		if has_kigo(arr, kigo)
  		keisan(arr, kigo)
		else
			arr
		end
